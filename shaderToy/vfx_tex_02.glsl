...
void mainImage(out vec4 O, in vec2 I){
  ...
  float n1 = noise(uv*1.4+vec2(0,-mod(T*.3, 100.)));
  vec2 uv2 = uv + n1;

  vec2 uv3 = mix(uv, uv2, .1);
  // vec2 uv3 = mix(uv, vec2(n1), .2);

  float n = fbm(uv3*4.*vec2(4.,1.)+vec2(0, mod(T*2., 100.)));
  // n = abs(n-.5);
  // n += .1;
  // n *= (n+.4);
  // n = pow(n, 2.);

  float mask_gradient = S(0.1,1.,uv.y);
  n *= mask_gradient;

  // float d = abs(length(uv-.5) - .2);
  // vec2 uv4 = s1(uv*TAU*4.+n*7.);
  // float d = min(uv4.x, uv4.y);
  float d = abs(sin(uv.x*20.+n*4.));
  // d += n*10.;
