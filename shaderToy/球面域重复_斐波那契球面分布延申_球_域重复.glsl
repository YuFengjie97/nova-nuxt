...
vec3 fibonacciSphere(float i, float N, float offset) {
  float phi = (1.0 + sqrt(5.0)) * 0.5;              // 黄金比例 ≈ 1.618
  // float goldenAngle = 2.0 * PI * (1.0 - 1.0/phi);   // ≈ 137.5°
  float goldenAngle = sin(T*.07+offset*.1)*PI*2.;
...
float map(vec3 p, float r, float id){
  ...
  float d = 1e5;
  for(float n=0.;n<N;n++){
    vec3 pos = fibonacciSphere(n,N,id);
    float d1 = length(p-pos*r)-.1;
    d = min(d, d1);
  }
  return d;
}
...
void mainImage(out vec4 O, in vec2 I){
  ...
  for(float i =0.;i<100.;i++){
    ...
    // repetition
    float s = 2.;
    float D = clamp(length(p),s*1., s*8.);  // limit range
    float id = round(D/s);
    vec3 q = p/id;
