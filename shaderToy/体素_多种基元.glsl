...
float map_voxel(vec3 ro, vec3 rd, inout vec3 vox){
...
vec3 calcNormal( in vec3 pos, float seed )
{
    vec2 e = vec2(1.0,-1.0);
    const float eps = 0.0005;
    return normalize( 
            e.xyy*map_el( pos + e.xyy*eps, seed ) + 
					  e.yyx*map_el( pos + e.yyx*eps, seed ) + 
					  e.yxy*map_el( pos + e.yxy*eps, seed ) + 
					  e.xxx*map_el( pos + e.xxx*eps, seed ) );
}
...
void mainImage(out vec4 O, in vec2 I){
  ...
  float steps = map_voxel(ro, rd, vox);
  float dep = steps/voxel_step_max;
  ...
  if(hit){
    ...
    vec3 nor = calcNormal(p, shape_seed);
