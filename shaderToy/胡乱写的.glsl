...
void mainImage(out vec4 O, in vec2 I){
  ...
  float life = tanh(sin(T*2.)*4.);
  // float life = fbm(vec2(1.,T))*2.;
  while(i++<80.){
    vec3 p = ro + rd * z;
    p.z += T*6.+life*2.;
    ...
    // p.xy *= rotate(life+p.z*.01);

    // vec3 q = vec3(atan(p.y,p.x), dot(p.xy, p.xy), p.z);
    vec3 q = vec3(atan(p.y,p.x), length(p.xy), p.z);
    ...
    q += sin(q.zxy-T*4.+life*2.)*.4;
    q += sin(q.zxy*2.)*.2;
    q += sin(q.zxy*8.)*.1;
    

    float d = sdBoxFrame(q - vec3(0,5,0), vec3(.8,2.,1.5), .2);
    // float d = sdBox(q - vec3(0,6.,0), vec3(.8,.4,.5));
    // float d = length(q-vec3(0,6,0))-1.2;
    d = abs(d)*.6 + .01;

    
    col += s1(C+q*.4+life*2.)/d;
