...
void mainImage(out vec4 O, in vec2 I){
  ...
  float nt = noise(mod(T*4.5, 60.)*.3) * 2.;
  ...
  for(float i=0.;i<100.;i++){
    ...
    float n;
    {
      vec3 q = p;
      q += cos(q.zxy*.1+nt)*4.;
      // q += cos(q.zxy*.1+nt*2.)*2.;
      // q += cos(q.zxy*.1+nt*4.)*1.;
      q.xy *= rotate(p.z*.1);
      d = abs(q.y) - PI * 2.;
    ...
    col += s1(vec3(3,2,1)+dot(p,vec3(.1))+nt*2.)*pow(1./d,2.);
    // col += s1(vec3(3,2,1)+dot(p, vec3(n*1.))+nt*.2)*pow(1./d,2.);
    // col += s1(vec3(3,2,1)+dot(p,p)*.01)*pow(1./d,2.);
