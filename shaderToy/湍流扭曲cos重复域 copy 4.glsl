...
void mainImage(out vec4 O, in vec2 I){
  ...
  for(float i=0.;i<100.;i++){
    ...
    {
      float t = mod(T, 60.);
      for(float j=1.;j<15.;j/=.6){
        q += .6*cos(q.yzx* j - vec3(0, 1., 4.)*t ) / j;
      }


      float planeScale = 2.*(tanh(sin(t+nt))*.5+.5);

      d = abs(q.y)*planeScale - 6. + dot(cos(q), sin(q.zxy*.2))*3.;
      d = abs(d)*.1 + .01;
    }

    
    col += s1(vec3(3,2,1)+dot(q,vec3(.1)))*pow(1./d,2.);
