...
vec2 radialShear(vec2 uv){
  float strength = .3;
  float ang = atan(uv.y, uv.x);
  ang *= 3.; // 后加
  float radius = length(uv); 
  radius = sin(radius)*2.; // 后加
  float offset = -iTime * 6.;
  float shearAmount = strength * radius * sin(radius*10. + offset);
  ang += shearAmount*.6;
...
void mainImage(out vec4 O, in vec2 I){
  ...
  float d1 = abs(sin(uv.y*2.));
  d = mix(d, d1, tanh(sin(T)*3.)*.5+.5);

  d = pow(.2/d, 2.);
  col += d*s1(vec3(3,2,1)+dot(uv,vec2(2)));
