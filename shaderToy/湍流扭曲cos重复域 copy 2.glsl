...
void mainImage(out vec4 O, in vec2 I){
  ...
  for(float i=0.;i<100.;i++){
    ...
    // p += cos(p.zxy*.1+tanh(sin(T*2.)+cos(T)*10.))*2.;
    p += cos(p.zxy*.1+nt*10.)*4.;
    p += cos(p.zxy*.1+nt*10.)*2.;
    p += cos(p.zxy*.1+nt*10.)*1.;

    p.xy *= rotate(p.z*.1);
    p.z -= T*25.;


    // float d = length(cos(p.xy)/1.4) - .1;
    float d = dot(cos(p),sin(p.zxy))-.1;

    {
      float d1 = abs(p.y) - PI*2.;
      d = max(d, -d1);
      // d = d1;
    }
    d = abs(d)*.4 + .01;
    ...
    col += s1(vec3(3,2,1)+dot(p,vec3(.1)))*pow(1./d,2.);
