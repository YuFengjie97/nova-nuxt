...
#iChannel0 "file://../public/sound/shaderToy_1.mp3"
// #iChannel0 "http://127.0.0.1:5500/shaderToy_1.mp3"
...
void mainImage(out vec4 O, in vec2 I){
  ...
  float freq = texture(iChannel0, vec2(.3,0.)).r;
  ...
  while(i++<80.){
    ...
    for(float j=0.;j<4.;j++){
      ...
      q.xy *= rotate(j*.5);
      q.yz *= rotate(j*.5);
      float d1 = sdBoxFrame(q, vec3(.4*j), .1*j);
      d = min(d, d1);
      // d = smin(d, d1, .8);
    }
