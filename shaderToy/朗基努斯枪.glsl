...
float range(float v,float start, float end,float k){
  return S(start, start+k, v) - S(end, end+k, v);
}
...
Obj map(vec3 p) {
  ...
  // 枪身缠绕
  float k0 = range(q.x,-len,2., .5);
  q.yz *= rotate(k0*q.x*4.);

  // 枪尾尖锐部分
  float k1 = S(x1, x1+5., q.x);
  thick *= k1;
  width *= k1;

  // 枪头螺旋分散部分
  float k3 = range(q.x, 0., 4., 1.);
  gap += k3*q.x*.2;

  // 两根枪头间距加大
  float k5 = range(q.x, 2., x2, 1.);
  gap += k5*1.;
  ...
  float d1 = sdRoundBox(q-vec3(0, vec2( gap,0.)), vec3(len, thick, width), .1);
  float d2 = sdRoundBox(q-vec3(0, vec2(-gap,0.)), vec3(len, thick, width), .1);
  float d = min(d1,d2);
...
void mainImage(out vec4 O, in vec2 I){
  ...
  // vec3 rd = setCamera(ro, vec3(0,0,0), 0.)*normalize(vec3(uv, 1.));
