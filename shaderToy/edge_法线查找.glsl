...
vec3 calcNormal(in vec3 p){
  ...
  float d2 = map(p-e.xyy).d;
  float d3 = map(p+e.yxy).d;
  float d4 = map(p-e.yxy).d;
  float d5 = map(p+e.yyx).d;
  float d6 = map(p-e.yyx).d;

  //float d = map(p).d*2.;
  // edge = abs(d1+d2-d) + abs(d3+d4-d) + abs(d5+d6-d);
  
  edge = abs(d1+d2) + abs(d3+d4) + abs(d5+d6);
  // edge = S(0., 1., sqrt(edge/eps*2.));
  edge = 1.-exp(-3.*edge/eps);
...
vec3 calcNormal2( in vec3 p )
{
    ...
    edge = abs(d1+d2-d) + abs(d1+d4-d) + abs(d1+d3-d);
    // edge = max(max(abs(d1+d2-d), abs(d1+d4-d)), abs(d1+d3-d));
    // edge = S(0., 1., sqrt(edge/eps*2.));
    edge = 1.-exp(-3.*edge/eps);
