...
void mainImage(out vec4 O, in vec2 I){
  ...
  for(float i=0.;i<100.;i++){
    ...
    vec3 q1 = p;
    vec3 q2 = p+vec3(.5,0,0);


    // big size repetition
    float id = floor(q1.x/2.);
    vec3 cen = vec3(id+.5, 0, 0)*2.;
    float d = length(q1-cen) - .5;
    d = max(0., d);    
    col += s1(vec3(3,2,1)+id+idd.y)/d;

    // small size repetition
    {
      float id = floor(q2.x);
      vec3 cen = vec3(id+.5, 0, 0);
      float d1 = sdBox(q2-cen, vec3(.2));
      d1 = max(0., d1);    
      d = min(d, d1);
      col += s1(vec3(3,2,1)+id)/d1;
    }
