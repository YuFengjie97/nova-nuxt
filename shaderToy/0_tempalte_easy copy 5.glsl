...
float noise(vec3 p){
  return dot(floor(cos(p*.2)*2.),vec3(.1,.2,.3));
}

vec3 col = vec3(0);
float map(vec3 p){
  p = abs(p);
  float d = abs(min(10.-p.x, 7.-p.y-.2*p.x)+.3);
  // float d1 = min(p.x, min(p.y, p.z));   
  // float d1 = min(p.x, p.y);
  // d -= d1;
  float a = max(0.,length(floor(sin(p*2.))));     
  float b = max(0.,length(ceil(sin(p/2.))));
  d -= a - b;
...
void mainImage(out vec4 O, in vec2 I){
  ...
  vec3 ro = vec3(0.,0.,T);
  vec3 rd = setCamera(ro, ro+vec3(0,0,1), 0.) * normalize(vec3(uv, 1.));
